...
Description:
Implements a modular, robust, and reproducible pipeline for exploring linked data as a network.
Enhanced data ingestion (support for RDF formats, SHACL validation, URI dereferencing),
basic RDF reasoning, ontology suggestions, schema mapping stubs, advanced SPARQL querying,
enriched graph visualization with semantic styling, an integrated probabilistic graph embedding model (via node2vec),
a new Node Similarity Search tab that computes cosine similarity between node embeddings, and
textual semantic analysis using SentenceTransformer.

WINNING SOLUTION IMPLEMENTED:
- **All custom CSS removed**. We rely entirely on Streamlit’s built-in theming (light/dark).
- No forced background or text color overrides. 
"""
...
def normalize_relationship_value(rel: str, value: Any) -> Optional[str]:
    if isinstance(value, dict):
        if rel in {
            "spouse", "studentOf", "employedBy", "educatedAt", "contributor",
            "draftsman", "creator", "owner", "dedicatee", "illustrator", "artist"
        }:
...
def build_graph(
    ...
    # Rely entirely on default theme
...
def create_legends(rel_colors: Dict[str, str], node_colors: Dict[str, str]) -> str:
    ...
    return (
        ...
        f"<strong>Relationships</strong>{rel_items}</ul>"
        f"<ul style='list-style: none; padding: 0;'>"
        f"<strong>Node Types</strong>{node_items}</ul></div>"
    )

def main() -> None:
    st.set_page_config(page_title="Linked Data Explorer", layout="wide")
    # (Winning Solution) No custom CSS block is injected; rely on Streamlit's theme.
    ...
    with st.sidebar.expander("Visualization Settings"):
        ...
        if preset_name != "Custom":
            ...
            st.session_state.physics_params["gravity"] = st.number_input(
                "Gravity",
                value=float(st.session_state.physics_params.get("gravity", CONFIG["PHYSICS_DEFAULTS"]["gravity"])),
                step=1.0,
                key="gravity_input"
            )
            st.session_state.physics_params["centralGravity"] = st.number_input(
                "Central Gravity",
                value=st.session_state.physics_params.get("centralGravity", CONFIG["PHYSICS_DEFAULTS"]["centralGravity"]),
                step=0.01,
                key="centralGravity_input"
            )
            st.session_state.physics_params["springLength"] = st.number_input(
                "Spring Length",
                value=float(st.session_state.physics_params.get("springLength", CONFIG["PHYSICS_DEFAULTS"]["springLength"])),
                step=1.0,
                key="springLength_input"
            )
            st.session_state.physics_params["springStrength"] = st.number_input(
                "Spring Strength",
                value=st.session_state.physics_params.get("springStrength", CONFIG["PHYSICS_DEFAULTS"]["springStrength"]),
                step=0.01,
            ...
            if nid_list:
                ...
                if node_obj:
                    with st.form("modify_node_form"):
                        ...
                        new_type_index = new_type_options.index(current_type) if current_type in new_type_options else 0
                        new_type_choice = st.selectbox("New Type", new_type_options, index=new_type_index)
    ...
    with tabs[0]:
        ...
        if st.session_state.graph_data.nodes:
            ...
            if st.session_state.selected_node:
                node_obj = next(
                    (n for n in st.session_state.graph_data.nodes if n.id == st.session_state.selected_node),
                    None
                )
                if node_obj:
